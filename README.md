...
| `gobuster_dir_scan` | Directory/file brute force | `target` (URL) | `gobuster dir -u <target> -w wordlist` |
| `gobuster_dns_scan` | DNS subdomain brute force | `target` (domain) | `gobuster dns -d <target> -w wordlist` |
| `gobuster_vhost_scan` | Virtual host brute force | `target` (URL) | `gobuster vhost -u <target> -w wordlist` |
